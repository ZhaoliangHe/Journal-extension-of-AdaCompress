...
%Therefore the next state is only  We use $P_{ss'}^{c} = P \big[s' \mid s, c = \pi(s) \big]$ to denote the state transition probability from state $s$ to $s'$, where $\pi$ is a compression strategy mapping state to actions.  
%
%We therefore obtain a sequence of states and actions $\{ s_1,c_1,s_2,...,c_{t-1},s_t \}$, and this formulation is a MDP.}
...
%The basic idea behind many reinforcement learning algorithms is to estimate the action-value function, by using the Bellman equation as an iterative update $ Q(s, c; \theta_{i}) = \mathbb{E}_{s' \sim \{\mathcal{X}, M\}} \big[ r + \gamma \max_{c'} Q(s', c'; \theta_{i-1}) \mid s, c \big] $. 	
	
%We denote the \emph{action-value function} as {\color{revise2} $ Q(\mathcal{E}(f_t), c; \theta) $} \iffalse $ Q(\phi(\mathcal{E}(f_t)), c; \theta) $ \fi and the optimal compression quality level at time $ t $ as {\color{revise2} $  c_t = {\rm argmax}_cQ(\mathcal{E}(f_t), c; \theta) $} \iffalse $ c_t = {\rm argmax}_cQ(\phi(\mathcal{E}(f_t)), c; \theta) $ \fi where $ \theta $ indicates the parameters of the Deep Q-learning Network $ \phi $.
